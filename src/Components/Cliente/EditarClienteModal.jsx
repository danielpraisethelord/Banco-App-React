...
export const EditarClienteModal = ({ isVisible, onClose, onEditSuccess, cliente }) => {
    ...
    const [sucursalId, setSucursalId] = useState(cliente?.sucursal?.id || "");
    ...
    useEffect(() => {
        if (cliente) {
            ...
            setRfc(cliente.rfc);
            setDiscapacidad(cliente.discapacidad);
            setEstadoCivil(cliente.estadoCivil);
            setNivelDeEstudios(cliente.nivelDeEstudios);
            setSucursalId(cliente.sucursal?.id || null);
        }
    }, [cliente]);
    ...
    const handleEditCliente = async (event) => {
        ...
        try {
            const newCliente = {
                ...
                estadoCivil,
                nivelDeEstudios,
                sucursal: sucursalId ? { id: sucursalId } : null
            }
            console.log('Cliente update', newCliente);
            const token = localStorage.getItem('token');
            await ClienteService.update(cliente.id, newCliente, token);
            ...
            >
                <div className="relative p-4 w-full max-w-2xl max-h-[80vh] bg-white rounded-lg shadow dark:bg-gray-700 overflow-y-auto custom-scrollbar">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <form onSubmit={handleEditCliente} className="p-4 md:p-5">
